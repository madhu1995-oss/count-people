...
def run():
	...
	while True:
		...
		# frame = cv2.resize(frame[1], (1280, 720), interpolation=cv2.INTER_CUBIC)
		# print(frame_src[1].shape)
		...
		for (objectID, centroid) in objects.items():
			...
			objectID_lis.append(objectID)

		# construct a tuple of information we will be displaying on the

		info1 = [
		("Count of unique people", objectID_lis),
		]

		# print(x)

		info2 = [
		("Count of unique people from left to right", totalUp),
		# ("Status", status),
		]
		...
		# 	for j in v:
		# 		text = "{}: {}".format(k, j)
		# 		cv2.putText(frame, text, (10, H - ((i * 20) + 60)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
	...
	fps.stop()
	print("\nCount of unique people: {}".format(objectID_lis[-1]))
	print("Count of unique people from left to right: {}".format(totalUp))
